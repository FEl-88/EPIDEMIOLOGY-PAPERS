...
clear mata
program define _FAYHeriotSae, eclass
	version 11.2
	#delimit ;
...
// load datasets into stata
mata: stata("use dataror, clear") 
mata: mata d // describe contents of the data 
 mata: st_nobs()
  mata: st_nvar() 
   mata: stata("renvars N / hh_size")
mata: mata set  matamofirst off
...
// begin with estimating EBLUP and MSE with Fh 
gen lnincome = ln(income)
mata: st_data(., .) 
mata: stata("fayherriot income unemployment, variance(directvariance) log biascorrection(sm) eblup(EBLUP1) mse(MSE1)")
// estimate shrinkage factor (GAMMA)
mata :stata("fayherriot income unemployment, variance(directvariance)  gamma") 
mata: beta1 = st_matrix("e(b)")  // store the contents of estimated covariates as beta1 
mata : beta1 
mata: vaEST = st_matrix("e(V)")
mata: vaEST
// fit fhsae which is extension of  fh eblup 
mata: stata("_FAYHeriotSae income unemployment hh_size, revar(directvariance) method(chandra)  areapredict(newarea)") 
mata: stata("ereturn list") 
glob sig = .95 // store the CI as 95%
disp $sig
...
// conformability 
mat I = I(`=rowsof(B)')  
// fit Foster Greer for poverty measure 
mata: stata("_FAYHeriotSae income unemployment hh_size, revar(incometax) method(fh)  nonegative maxiter(200) precision(1e-15)  fhse(fh_fgt0_se)  fhcv(fh_fgt0_cv) gamma(fh_fgt0_gamma)  fh(
> fh_fgt0)  out") 
mata: mata set matacache 2000  
